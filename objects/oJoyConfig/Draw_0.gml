...
if (!gamepadFound) {
    ...
    strLen = string_length(tr("NO GAMEPAD FOUND!"))*global.fontLargeWidth;
    n = 160 - strLen;
    n = ceil(n / 2);
    draw_text(n, 70, string_hash_to_newline(tr("NO GAMEPAD FOUND!")));
} else {
	if (status == -1) {
	    draw_set_font(global.fontLarge);
	    ...
	    n = 160 - strLen;
	    n = ceil(n / 2);
	    draw_text(n, 70, string_hash_to_newline(tr("PRESS ANY BUTTON")));
	} else {
	...
	if (currVal != "") {
		...
		draw_text(8+strCurrLen, 104, string_hash_to_newline(scrGetJoy(currVal)));
	}
}